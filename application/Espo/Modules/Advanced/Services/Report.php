...
use Espo\{
    ...
    Modules\Advanced\Core\ORM\SthCollection2,
...
class Report extends \Espo\Services\Record
{
    ...
    {
        if (isset($this->recordServiceContainer)) {
            ...
            if (strpos($item, '.') === false) {
                ...
                if (!empty($defs['relations']) && !empty($defs['relations'][$link])) {
                    ...
                    if ($foreignFieldType == 'enum') {
                        $this->fillEnumGroupNames($foreignScope, $field, $groupValueMap, $argument);
                    }
    ...
    protected function fillEnumGroupNames($entityType, $item, &$groupValueMap, ?string $key = null)
    {
        if ($key === null) {
            $key = $item;
        }

        $groupValueMap[$key] = $this->getLanguage()->translate($item, 'options', $entityType);

        if (!is_array($groupValueMap[$key])) {
            unset($groupValueMap[$key]);

            $translation = $this->getMetadata()->get(['entityDefs', $entityType, 'fields', $item, 'translation']);

            if ($translation) {
                $groupValueMap[$key] = $this->getLanguage()->get(explode('.', $translation));
    ...
    protected function getForeignAttributeType(string $entityType, string $link, string $attribute): ?string
    {
        $metadata = $this->getEntityManager()->getMetadata();
        ...
        $foreignType =
            $metadata->get($foreignEntityType, ['attributes', $attribute, 'type']) ??
    ...
    protected function executeListReport(
        ...
        if (
            ...
            $collectionClassName = ($this->isDevVersion() || version_compare($version, '7.0.0') >= 0) ?
                SthCollection2::class :
                SthCollection::class;
    ...
    protected function executeGridReport($entityType, $data, $where, $additionalParams = [], $user = null)
    {
        ...
        if (!empty($params['havingClause']) && empty($params['groupBy'])) {
            throw new Error("Having filter requires non-empty Group By.");
        }
        ...
        if ($useSubQuery) {
            ...
            $this->handleColumns($aggregatedColumnList, $entityType, $params, $stub1);
        ...
        foreach ($params['groupBy'] as $i => $groupCol) {
            ...
            if (count($data->groupBy) === 2 && $data->orderBy) {
                $this->orderGrouping($entityType, $groupCol, $data->orderBy[0], $grouping[$i], $rows);
            }
        }
        ...
        $result = [
            ...
            'columnDecimalPlacesMap' => (object) $columnDecimalPlacesMap,
        ];
    ...
    protected function executeGridReportSubListItem(
        ...
        if (isset($data->orderBy)) {
            foreach ($data->orderBy as $orderByItem) {
        ...
        $this->handleOrderBy($newOrderBy, $data->entityType, $selectParams, $stubOrder);
    ...
    private function orderGrouping(
        ...
        $columnData = $this->getDataFromColumnName($entityType, $groupCol);
        ...
        foreach ($rows as $i => $rowItem) {
            $itemColumn = $rowItem[$groupCol];
            $itemValue = $rowItem[$orderBy];
    ...
    protected function executeJointGridReport($data, $user = null, $params = [])
    {
        ...
        foreach ($data->joinedReportDataList as $i => $item) {
            ...
            foreach ($subReportResult['columnDecimalPlacesMap'] as $key => $type) {
                $columnTypeMap[$key . '@'. $i] = $type;
            }

            $subReportResult['columnDecimalPlacesMap'] = $columnDecimalPlacesMap;
    ...
    public function populateTargetList($id, $targetListId)
    {
        ...
        $linkList = $this->getMetadata()->get(['scopes', 'TargetList', 'targetLinkList']) ??
        ...
        foreach ($linkList as $itemLink) {
            if (
                $this->getMetadata()->get(['entityDefs', 'TargetList', 'links', $itemLink, 'entity']) === $entityType
            ) {
                $link = $itemLink;
        ...
        if (!$link) {
            throw new Error("Not supported entity type for target list sync.");
        }
    ...
    public function exportList($id, $where = null, array $params = null, $user = null)
    {
        ...
        if (
            method_exists($this->aclManager, 'getPermissionLevel') &&
            $this->aclManager->getPermissionLevel($user ?? $this->getUser(), 'exportPermission') === 'no'
        ) {
    ...
    protected function getCustomEntityFactory(): CustomEntityFactory
    {
        if (!$this->customEntityFactory) {
            $injectableFactory = $this->injectableFactory ?? $this->getContainer()->get('injectableFactory');
    ...
    private function isDevVersion(): bool
    {
        $version = $this->getConfig()->get('version');

        return $version === 'dev' || $version === '@@version';
    }
}
