    ...
    public function updateAccounts($clientiURL) {
        ...
        if(!empty($jobId)) {
            foreach ($clienti as $cliente) {
                //Account
                $account = $this->entityManager->getRepository('Account')->where(['anCodice' => trim($cliente->ancodice)])->findOne();
                if (!$account || $account->get('cpccchk') != trim($cliente->cpccchk)) {
                    // New or modified
                    $account = !$account ? $this->entityManager->getEntity('Account') : $account;
                    $account->set(array(
            ...
            // Deleted
            $accountList = $this->entityManager->getRepository('Account')->where([
                [
                    'AND' => [
                        'idJob!=' => $jobId,
                        'anCodice!=' => null
    ...
    public function updateProducts($articoliURL) {
        ...
        if(!empty($jobId)) {
            foreach ($articoli as $articolo) {
                // ProductCategory
                $category = $this->entityManager->getRepository('ProductCategory')->where(['arCatCon' => trim($articolo->arcatcon)])->findOne();
                if (!$category || $category->get('idJob') != $jobId) {
                    $category = !$category ? $this->entityManager->getEntity('ProductCategory') : $category;
                    $category->set(array(
                ...
                // Product
                $product = $this->entityManager->getRepository('Product')->where(['arCodArt' => trim($articolo->arcodart)])->findOne();
                if (!$product || $product->get('cpccchk') != trim($articolo->cpccchk)) {
                    // New or modified
                    $product = !$product ? $this->entityManager->getEntity('Product') : $product;
                    $product->set(array(
            ...
            // Deleted
            $productList = $this->entityManager->getRepository('Product')->where(['idJob!=' => $jobId])->find();
    ...
    public function updatePrepagati($prepagatiURL) {
        ...
        if(!empty($jobId)) {
            foreach ($prepagati as $prepagato) {
                // Prepagato
                $prep = $this->entityManager->getRepository('Prepagato')->where(['mvSerial' => trim($prepagato->mvserial)])->findOne();
                if (!$prep || $prep->get('cpccchk') != trim($prepagato->cpccchk)) {
                    // New or modified
                    $prep = !$prep? $this->entityManager->getEntity('Prepagato') : $prep;
                    $account = $this->entityManager->getRepository('Account')->where(['anCodice' => trim($prepagato->mvcodcon)])->findOne();
                    $prep->set(array(
                ...
                $this->entityManager->saveEntity($prep);
            }
            // Deleted
            $prepagatoList = $this->entityManager->getRepository('Prepagato')->where(['idJob!=' => trim($jobId)])->find();
            foreach ($prepagatoList as $prepagato) {
                $prepagato->set('deleted', 1);
                $this->entityManager->saveEntity($prepagato);
                $deleted['n'] += 1;
