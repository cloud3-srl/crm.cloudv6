...
use Espo\Modules\Sales\Repositories\Quote as QuoteRepository;
use Espo\ORM\Entity;
...
class Quote extends \Espo\Services\Record
{
    ...
    public function loadItemListField(Entity $entity): void
    {
        ...
        $repository->loadItemListField($entity);
    }
    ...
    protected function getAttributesFromAnotherRecord($sourceType, $sourceId)
    {
        ...
        $sourceItemList = $this->getEntityManager()
            ->getRepository($sourceItemType)
            ->where([
        ...
        foreach ($sourceItemList as $item) {
            ...
            foreach ($itemSeed->getAttributeList() as $attribute) {
                ...
                if (array_key_exists($attribute, $itemAttributes)) {
                    continue;
                }
    ...
    public function getAttributesForEmail($sourceId, $templateId, array $params = [])
    {
        ...
        if (
            !$this->getAcl()->checkEntity($quote, 'read') ||
            !$this->getAcl()->checkEntity($template, 'read')
        ) {
            throw new Forbidden();
        }
        ...
        $subjectTpl = $this->getTemplateFileManager()
            ->getTemplate('salesEmailPdf', 'subject', $quote->getEntityType(), 'Sales');

        $bodyTpl = $this->getTemplateFileManager()
        ...
        $subject = $htmlizer->render(
            $quote,
            $subjectTpl,
            'sales-email-pdf-subject-' . $quote->getEntityType(),
            $data,
            true
        );
        ...
        $body = $htmlizer->render(
            $quote,
            $bodyTpl,
            'sales-email-pdf-body-' . $quote->getEntityType(),
            $data,
            false
        );
    ...
    public function getConvertCurrencyValues(
        ...
        $data = parent::getConvertCurrencyValues(
            $entity,
            $targetCurrency,
            $baseCurrency,
            $rates,
        ...
        if (
            ...
            foreach ($this->getFieldManagerUtil()->getEntityTypeFieldList($this->itemEntityType) as $field) {
                if ($this->getMetadata()->get(['entityDefs', $this->itemEntityType, 'fields', $field, 'type']) !==
                    'currency'
                ) {
                    continue;
                }
            ...
            foreach ($itemCollection as $item) {
                $values = $itemService->getConvertCurrencyValues(
                    $item,
                    $targetCurrency,
                    $baseCurrency,
                    $rates,
