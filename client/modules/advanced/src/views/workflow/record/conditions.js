...
define('advanced:views/workflow/record/conditions', ['view', 'advanced:workflow-helper'], function (Dep, Helper) {
    ...
    return Dep.extend({
        ...
        addCondition: function (conditionType, field, data, isNew) {
            ...
            var fieldNameHtml = '<label class="field-label-name control-label small">' + escapedLabel + '</label>';
            var removeLinkHtml = this.readOnly ? '' :
                '<a role="button" tabindex="0" class="pull-right" data-action="removeCondition" data-id="' + escapedId + '">' +
                '<span class="fas fa-times"></span></a>';

            var html = '<div class="cell form-group" style="margin-left: 20px;">' +
                removeLinkHtml + fieldNameHtml + '<div class="condition small" data-id="' + escapedId + '"></div></div>';
