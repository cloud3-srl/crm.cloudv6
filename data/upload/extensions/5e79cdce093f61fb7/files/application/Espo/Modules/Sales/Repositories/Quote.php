...
class Quote extends \Espo\Core\ORM\Repositories\RDB
{
    ...
    protected function beforeSave(Entity $entity, array $options = [])
    {
        ...
        if ($entity->isNew() && $this->getMetadata()->get(['entityDefs', $this->entityType, 'fields', 'number', 'useAutoincrement'])) {
    ...
    protected function processItemsAfterSave(Entity $entity, array $options = [])
    {
        ...
        if (!$entity->has('itemList')) {
            if ($entity->isAttributeChanged('accountId')) {
                $quoteItemList = $this->getEntityManager()->getRepository($this->itemEntityType)->where([
        ...
        if (!$entity->isNew()) {
            $prevItemCollection = $this->getEntityManager()->getRepository($this->itemEntityType)->where(array(
                $this->itemParentIdAttribute => $entity->id
            ))->order('order')->find();
    ...
    protected function setItemWithData(Entity $item, \StdClass $o)
    {
        ...
        $ignoreAttributeList = [
            ...
            $this->itemParentIdAttribute,'listPriceConverted', 'unitPriceConverted', 'amountConverted', 'deleted'
        ];
